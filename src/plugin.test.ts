...
test("parseOperations extracts descriptor-relevant operation details", () => {
  const operations = parseOperations(sampleSpec, defaultConfig);
...
test("parseOperations applies include/exclude/tags/method filters together", () => {
  ...
  const operations = parseOperations(sampleSpec, config);
...
test("parseOperations keeps optional object request bodies nested under body", () => {
...
test("parseOperations includes header and cookie parameters in descriptor inputs", () => {
  const operations = parseOperations(
    {
      openapi: "3.0.0",
      paths: {
...
test("parseOperations lets operation-level parameters override path-level parameters", () => {
  const operations = parseOperations(
    {
      openapi: "3.0.0",
      paths: {
...
test("generateDescriptors throws when normalized tool names collide", () => {
  const operations = parseOperations(
    {
      openapi: "3.0.0",
      paths: {
  ...
  assert.throws(
    () => generateDescriptors(operations, defaultConfig),
    /Duplicate normalized toolName "get_user_by_id"/
  );
});

test("generateDescriptors creates descriptor-only output", () => {
  const operations = parseOperations(sampleSpec, defaultConfig);
  const code = generateDescriptors(operations, defaultConfig);
...
test("generateDescriptors keeps metadata disabled by default", () => {
  const operations = parseOperations(sampleSpec, defaultConfig);
  const code = generateDescriptors(operations, defaultConfig);
...
test("generateDescriptors includes only selected metadata fields", () => {
  const config: ResolvedConfig = {
    ...defaultConfig,
    metadata: {
      enabled: true,
  ...
  const operations = parseOperations(sampleSpec, config);
  const code = generateDescriptors(operations, config);
...
test("generateDescriptors allows metadata transforms to override derived metadata", () => {
  const config: ResolvedConfig = {
    ...defaultConfig,
    metadata: {
      enabled: true,
...
test("generateDescriptors emits direct Vercel AI SDK tools when provider integration is enabled", () => {
  const config: ResolvedConfig = {
    ...defaultConfig,
    providers: {
      vercelAiSdk: {
...
test("fixture specs preserve descriptor edge cases", () => {
  const optionalBodyOperation = parseOperations(
  ...
  const headerCookieOperation = parseOperations(
  ...
  const overrideOperation = parseOperations(readFixtureSpec("override"), defaultConfig)[0];
  assert.ok(overrideOperation);
  assert.deepEqual(overrideOperation.inputSchema.properties, {
    id: { type: "integer" },
  });

  const collisionOperations = parseOperations(readFixtureSpec("collision"), defaultConfig);
  assert.throws(
    () => generateDescriptors(collisionOperations, defaultConfig),
    /Duplicate normalized toolName/
  );
});

test("inferIntent infers read/search/create/update/delete/workflow", () => {
  assert.equal(inferIntent("get", "getUserById"), "read");
  assert.equal(inferIntent("get", "listUsers"), "search");
  assert.equal(inferIntent("post", "createUser"), "create");
  assert.equal(inferIntent("post", "recomputeJobs"), "workflow");
  assert.equal(inferIntent("patch", "updateUser"), "update");
  assert.equal(inferIntent("delete", "deleteUser"), "delete");
});

test("inferSafety marks dangerous and confirm operations", () => {
  assert.equal(inferSafety("get", "getUser"), "safe");
  assert.equal(inferSafety("delete", "deleteUser"), "dangerous");
  assert.equal(inferSafety("post", "forceCloseOrder"), "dangerous");
  assert.equal(inferSafety("post", "createUser"), "confirm");
});

test("extractNouns and route description keep metadata heuristics reusable", () => {
  const nouns = extractNouns("/api/v1/orders/{id}/items", "getOrderItems");

  assert.ok(nouns.includes("order"));
  assert.ok(nouns.includes("item"));

  const routeDescription = buildRouteDescription("search", "List orders", nouns, ["orders"]);
  assert.ok(routeDescription.includes("[search]"));
  assert.ok(routeDescription.includes("Entities:"));
  assert.ok(routeDescription.includes("Tags:"));
});
